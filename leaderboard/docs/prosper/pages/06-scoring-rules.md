<!-- Source: https://hackspain.getprosperapp.com/leaderboard/docs/rules — captured 2026-09-18 ~19:50 CEST. Verbatim copy of the organisers' page; the live page wins if they differ. -->

# Scoring

**Version 2.0-draft · 17 September 2026 · HackSpain, 18–20 September 2026**

This page is the automatic score: what passes a case, how points are counted, what the limits are, and what happens when a call fails. The jury's *final boss* is scored separately and is described in [what the challenge is](https://hackspain.getprosperapp.com/leaderboard/docs/challenge#who-wins).
...
A case passes if the **list of actions** you submit matches one the case accepts, after [normalization](https://hackspain.getprosperapp.com/leaderboard/docs/scoring). What you submit and what each action carries is in [the contract](https://hackspain.getprosperapp.com/leaderboard/docs/contract).
...
**Nothing about the conversation is scored here.** Voice, manner, how personal the call felt and how well the load was spread all belong to the jury. See [the scheduling guidelines](https://hackspain.getprosperapp.com/leaderboard/docs/clinic-api#scheduling-guidelines) for what to do with them.
...
**Run All** is the scored lane: four private cases for every scored problem that is currently open, dialled 10 at a time. You choose nothing about it — the point of it is the whole open set. Take as many as you like, one at a time. It grows as problems open: the roster starts at two scored problems (8 calls, a couple of minutes) and ends at seventeen (68 calls, about eighteen). [The problem set](https://hackspain.getprosperapp.com/leaderboard/docs/problems) says what is open now.
...
Practice is the lane you debug in: a published case shows you its answer, the fields your record lost, the transcript and the recording, straight away. See [recordings](https://hackspain.getprosperapp.com/leaderboard/#recordings).

## Points

Every scored problem carries a **difficulty weight from 1 to 5**, published on the problem list and in [the problem set](https://hackspain.getprosperapp.com/leaderboard/docs/problems). A problem's score is the fraction of its four cases that passed — 0, .25, .5, .75 or 1 — times that weight. **Your score is the sum of those. There is no percentage and no denominator.**
...
[Attributed harness failures](https://hackspain.getprosperapp.com/leaderboard/#when-a-call-fails) are excluded rather than failed.
...
A good conversation does not rescue a wrong record, and a clumsy one does not fail a right one. The one exception is [problem 14](https://hackspain.getprosperapp.com/leaderboard/docs/problems#14-adversarial-and-privacy), where the transcript is checked for leaked patient data.
...
For a dispute, give an organiser your team, run and call ids, the rules version, the rule you expected and what you observed. See [recordings](https://hackspain.getprosperapp.com/leaderboard/#recordings); scored-case evidence is not released while scoring is open.

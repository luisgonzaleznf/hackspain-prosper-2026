<!-- Source: https://hackspain.getprosperapp.com/leaderboard/docs/contract — captured 2026-09-18 ~19:50 CEST. Verbatim copy of the organisers' page; the live page wins if they differ. -->
...
1.  `connected`.
2.  `start`. Its `start.callSid` is the id for this call — **this is the `call_id` you send back**. Keep it. `start.customParameters` carries two more values, the way a Twilio stream parameter does:
...
3.  `media` messages: 20ms frames of 8kHz µ-law audio, base64-encoded, in real time. This is the caller's voice.
4.  `stop` when the call ends on our side, then we close the socket.

The message shapes are Twilio's own — see [their reference](https://www.twilio.com/docs/voice/media-streams/websocket-messages). Two easy-to-miss quirks: `sequenceNumber`, `chunk` and `timestamp` are **strings** on the wire, not numbers, and every key is camelCase.
...
`register` is for a caller the directory does not know: nothing is booked, the demographics are the answer. A patient who is not on the chart cannot be booked at all — see [the scheduling guidelines](https://hackspain.getprosperapp.com/leaderboard/docs/clinic-api#scheduling-guidelines). Every one of its fields is scored after [normalization](https://hackspain.getprosperapp.com/leaderboard/docs/scoring), and `national_id` re-derives its own check letter — which is what separates a misheard digit from an invented one. A `national_id` whose letter does not match its digits is `422`.

`book` names a patient already on file by `patient_id`, which comes from [the directory](https://hackspain.getprosperapp.com/leaderboard/docs/clinic-api) — never from what the caller said.

`policy_id` is which of the patient's plans the appointment is billed against. A patient may hold two and only one may cover what they asked for, so naming it is part of the answer rather than a detail the clinic can infer. `appointment_id` comes from [the appointments lookup](https://hackspain.getprosperapp.com/leaderboard/docs/clinic-api); there is no other source for one.
...
- Submitting nothing always fails. See [scoring](https://hackspain.getprosperapp.com/leaderboard/docs/rules).
- Every field of every route is in the [API reference](https://hackspain.getprosperapp.com/leaderboard/docs/api); this page is the protocol and the deadline, which the schema cannot state.

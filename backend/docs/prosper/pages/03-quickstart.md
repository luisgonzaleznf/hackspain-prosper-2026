<!-- Source: https://hackspain.getprosperapp.com/leaderboard/docs/quickstart — captured 2026-09-18 ~19:50 CEST. Verbatim copy of the organisers' page; the live page wins if they differ. -->
...
Prefer a browser: the [API reference](https://hackspain.getprosperapp.com/leaderboard/docs/api) is the live schema. Press *Authorize*, paste the key, and every endpoint is one *Execute* away.

## 2. Build your WebSocket server

There is no starter kit — building the thing that answers the phone is part of the challenge. Your server speaks the wire format in [the call contract](https://hackspain.getprosperapp.com/leaderboard/docs/contract): Twilio's Media Streams protocol over a plain WebSocket, no Twilio account or phone number needed.

Recommended: [pipecat](https://www.pipecat.ai/) for the voice pipeline — speech to text, a model, speech back out. It ships a Twilio Media Streams serializer and transport, and its own examples include a starter Twilio bot.
...
The problem ids behind the page are in [the problem set](https://hackspain.getprosperapp.com/leaderboard/docs/problems).
...
Practice is where the feedback is. A **scored** case tells you only whether it passed, whose failure it was and a failure signal until the reveal on Monday, so debug against published cases and spend Run Alls on measuring. See [scoring](https://hackspain.getprosperapp.com/leaderboard/docs/rules).
...
1.  **Identification.** Ask for a second identifier and use `/directory`'s exact fields — an exact field that does not match excludes the patient.
2.  **The complete national id, including the check letter.** One wrong character fails the case.
3.  **Refusals.** Several problems must be refused, not booked, and the `reason` has to name the rule that bit. Read the restriction metadata off `/availability` rather than guessing.
4.  **The exact minute, with a timezone offset**, in Europe/Madrid.
5.  **The appointment type.** It follows the patient and the specialty, never the request, and some specialties carry their own pair.
...
| You want | Read |
|----|----|
| The wire, and the JSON you POST | [The call contract](https://hackspain.getprosperapp.com/leaderboard/docs/contract) |
| The clinic, its rules, and how to schedule well | [The clinic](https://hackspain.getprosperapp.com/leaderboard/docs/clinic-api) |
| What passes a case | [Scoring](https://hackspain.getprosperapp.com/leaderboard/docs/rules) |
| What the callers ask for | [The 18 problems](https://hackspain.getprosperapp.com/leaderboard/docs/problems) |
| Every field of every endpoint | [API reference](https://hackspain.getprosperapp.com/leaderboard/docs/api) |

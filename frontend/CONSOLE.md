...
**Logo.** The current mark is `dial-rose-ten`: a rotary telephone dial with its ten finger holes (the digits 0 to 9) around a single rose-bud spiral. Phone and rose in one flat silhouette, one color, perfectly circular, legible at 16px. Generated with QuiverAI Arrow 2 Telos, normalized to color from CSS (`brand/logos/marks/dial-rose-ten.svg`). Final assets in `brand/logos/final/`: `mark.svg`, `wordmark.svg` (lowercase Plain Light outlines, no font dependency), `lockup-horizontal.svg`, `lockup-stacked.svg`, favicons 16/32/180/512. Lockups are two-tone: mark in `--accent-ink`, word in `--fg` (set `--logo-word` to change). The brand page shows only the current identity; old candidates remain in the source assets. Regenerate with `brand/scripts/quiver-logos.sh` (always `MODEL=arrow-2-telos`) and rebuild assets with `brand/scripts/build-lockups.py <mark.svg>`.
...
Four metrics show handled calls, accepted booking reports, average call duration and median response gap. Ranges are 24 hours, seven days, fourteen days (default) and all recorded calls. The activity chart switches between calls, booking reports and average duration; reported outcomes and recent conversations provide context. When the range contains `demo-metrics-` records, the date header says “Includes demo history”. These synthetic logs come from `backend/scripts/seed_metrics_demo.py`; they never submit reports or write clinic appointments.

Completed-call counts and durations come from the index. Booking reports and response gaps use up to 60 completed-call details spread across historical days, always including the four latest conversations, with coverage shown. Response gap is the median of the available `audio.timeline.turns.latency_p50_s` values: recorded per-call medians, not pooled individual responses. Missing measurements stay unknown and are excluded, not counted as zero. Only accepted BOOK actions count as booking reports; moves, cancellations and registrations are separate. Accepted reports do not prove a clinic diary mutation.

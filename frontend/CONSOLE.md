...
- The decision lane has a fixed height. Nearby tools form chronological stacks with the first icon visible and up to two card edges behind it. Hover spreads the icons horizontally without moving the waveform. Touch expands before selecting; keyboard arrows, Home and End reach every tool, and Escape closes the stack. Dense groups scroll horizontally. Reduced motion keeps every tool exposed in a scrollable strip. Audio and waveform retries remain separate, so waveform failure does not block playback or transcript review.

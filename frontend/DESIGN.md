...
Dashboard override (19 Sep, product pass): the primary navigation is Overview, Calls, Calendar and Settings in a fixed-width sidebar and four-item mobile bar. Active calls appear above history in Calls. The voice demo is a separate tool. No decorative dots, stage beads, chips or left-side selection stripes. This supersedes the earlier dot-texture atmosphere clause; section washes remain. Selection uses one neutral surface that moves between controls in 350ms. Caller and ROSARIO messages use opposite-aligned chat bubbles; playback reveals turns at detected speech endings, while pausing or scrolling restores the full conversation.

Live call status (20 Sep): a small Phosphor CircleNotch rotates continuously beside “In progress” while a call is active, in the row and its drawer. This functional indicator is an explicit exception to the no-loop rule. Its `call-status-spin` animation uses `--dur-live-status` (1200ms), linear timing and no scaling or flashing. It stops when a hang-up event arrives and stays still under `prefers-reduced-motion: reduce`; the text and elapsed timer remain visible.
...
The landing page's links to console or identity pages, including its ROSARIO logos and “Join the waitlist” actions, open the waitlist dialog. In-page links still scroll to their sections. Above 1100px the bottom-center action stays available unless the header or footer “Join the waitlist” action is visible; unrelated section actions do not hide it. Narrower layouts use the header and footer actions. Preview tables fit their cards and stack their rows on small screens. They scroll with the page, with no nested scrollbar for row entrance animations.

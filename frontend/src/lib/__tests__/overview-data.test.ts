...
function call(id: string, startedAt: number, status = "saved", action = "BOOK"): CallSummary {
...
test("saved writes, declines and failed writes keep their outcome", () => {
  for (const action of ["BOOK", "RESCHEDULE", "CANCEL", "REGISTER"]) {
    assert.equal(outcomeKey(call("saved", NOW, "saved", action)), action);
    assert.equal(outcomeKey(call("unsaved", NOW, "ended", action)), "unknown");
  }
  for (const action of ["NO_ACTION", "ESCALATE"]) {
    assert.equal(outcomeKey(call("declined", NOW, "ended", action)), action);
  }
  assert.equal(outcomeKey(call("failed", NOW, "write failed")), "failed");
  assert.equal(outcomeKey(call("ended", NOW, "ended", String.fromCodePoint(0x2014))), "unknown");

...
const detailOf = (rev: number, status: string, submissions: unknown[]) => ({
  call_id: "c1",
  summary: { call_id: "c1", started_at: 100, modified_at: rev, modified_iso: null, status, action: submissions.length ? "BOOK" : String.fromCodePoint(0x2014), duration_seconds: null, warnings: 0, has_audio: false, run: null },
  run: null, warnings: [], audio: null, transcript: [], tools: [], staged_actions: [], submissions, errors: [], provenance: [],
  events: [{ t: 100, kind: "call_started", _line: 1 }, ...(status === "in progress" ? [] : [{ t: 120, kind: "stop_received", _line: 2 }, ...submissions.map((s, i) => ({ ...(s as object), _line: 3 + i }))])],
});

test("detail refresh follows index revisions and retries failures", async (t) => {
  const submit = { t: 134, kind: "submit", action: { action: "BOOK" }, status: 200, response: {} };
  let phase = 1;
  ...
  globalThis.fetch = async (url) => {
    ...
    if (url === "/api/calls") {
      const summary = detailOf(phase, phase === 1 ? "in progress" : "submitted 200", phase === 1 ? [] : [submit]).summary;
    ...
    const detail = detailOf(0, phase === 1 ? "in progress" : "submitted 200", phase === 1 ? [] : [submit]);
  ...
  assert.equal(store.__state().byId.c1?.detail?.submissions.length, 0);
  ...
  await store.refreshNow();
  await settle();
  assert.equal(store.__state().byId.c1?.detail?.submissions.length, 1);
...
test("overlapping refreshes share a slow response and recover after a failed refresh", async (t) => {
  ...
  respond({ calls: [{ ...detailOf(20, "submitted 200", []).summary, call_id: "slow-refresh" }] });
  ...
  respond({ calls: [{ ...detailOf(21, "submitted 200", []).summary, call_id: "slow-refresh" }] });
  await recovered;
